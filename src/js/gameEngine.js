...
function modifyPlayerAvatarPosition(state, game){
    //moving playerAvatar
   const {playerAvatar} = state; 
   game.playerAvatarElement.style.backgroundImage = 'url("./images/player.png")';
   if(state.keys.KeyD){
       playerAvatar.posX = Math.min(playerAvatar.posX + playerAvatar.speed, game.gameScreen.offsetWidth - playerAvatar.width);
       game.playerAvatarElement.style.backgroundImage = 'url("./images/playerRight.png")';
   }
   if(state.keys.KeyA){
       playerAvatar.posX = Math.max(playerAvatar.posX - playerAvatar.speed,0);
       game.playerAvatarElement.style.backgroundImage = 'url("./images/playerLeft.png")';
   }
   if(state.keys.KeyW){
